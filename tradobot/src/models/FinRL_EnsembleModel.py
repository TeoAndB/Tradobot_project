...
from src import config_model

MODELS = {"a2c": A2C, "ddpg": DDPG, "td3": TD3, "sac": SAC, "ppo": PPO}

MODEL_KWARGS = {x: config_model.__dict__[f"{x.upper()}_PARAMS"] for x in MODELS.keys()}
...
class DRLEnsembleAgent:
    ...
    def get_model(
        ...
        return MODELS[model_name](
            policy=policy,
            env=env,
            tensorboard_log=f"{config_model.TENSORBOARD_LOG_DIR}/{model_name}",
            verbose=verbose,
            policy_kwargs=policy_kwargs,
            seed=seed,
            **temp_model_kwargs,
    ...
    def train_model(model, model_name, tb_log_name, iter_num, total_timesteps=5000):
        ...
        model.save(
            f"{config_model.TRAINED_MODEL_DIR}/{model_name.upper()}_{total_timesteps // 1000}k_{iter_num}"
        )
        return model

...
def main(input_filepath, output_filepath):
    ...
    selected_data_entries = 'HuberLoss_wDropout_weight-decay_increased_exploration_2nd_run'
    ...
    for e in range(agent.num_epochs):
        ...
        for t in range(l_training):
            ...
            # scalp every 100$
            if agent.portfolio_state - agent.initial_total_balance > 100:
                    agent.sell_everything(closing_prices, stock_i, h, e, dates)
                    NEW_INITIAL_AMOUNT = INITIAL_AMOUNT + (agent.portfolio_state[0,0] - INITIAL_AMOUNT)
                    agent = Agent(num_stocks=NUM_STOCKS, actions_dict=ACTIONS_DICTIONARY, h=h, num_features=num_features, balance=NEW_INITIAL_AMOUNT, name_stocks=cols_stocks,
                                  gamma=GAMMA, epsilon=EPSILON, epsilon_min=EPSILON_MIN,
                                  epsilon_decay=EPSILON_DECAY, learning_rate=LEARNING_RATE, batch_size=BATCH_SIZE, tau=TAU, num_epochs=NUM_EPOCHS)
            ...
            next_day = next_data_window.loc[0, 'date'].day
            old_day = data_window.loc[0, 'date'].day

            agent.remember(state=state, actions=(action_index_for_stock_i, stock_i, h),
            ...
            if old_day != next_day:
                ...
                day_loss_history = agent.batch_loss_history.copy()
                train_loss_history_per_day.extend(day_loss_history)
                ...
                print(f'Day {e+1}, Training Loss: {loss_per_epoch_log:.4f}')
